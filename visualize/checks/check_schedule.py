...
def formater(df, measurement):
    ...
    lines = [{"measurement": f"{measurement}", "fields": {row["_field"]: float(row["_value"])}, "time": row["_time"]} for _, row in df.iterrows() if row["_value"] != 0 and not math.isnan(row["_value"])]
    return lines
