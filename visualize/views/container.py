...
def render_transient_report(device='mp'):
  ...
  if sess("data") is not None:
    ...
    tab2, tab1 = st.tabs(['Analyse','Report'])
    with tab1:
    ...
    with tab2:
      if filtered_df is not None and not filtered_df.empty:
        ...
        draw_barchart(filtered_df,
                      ...
                      height=height,
                      col_num=col_num,
                      #domain=[parser.isoparse(f"{sess('start_date')}T{sess('start_time')}+07:00"), parser.isoparse(f"{sess('end_date')}T{sess('end_time')}+07:00")]
        )
