...
def load_tag_config(filename="assets/files/tags.yaml"):
  with open(filename, "r") as yaml_file:
    control_logic_checks, deviation_checks, devices = itemgetter(
...
@st.cache
def load_tag_config_new(filename="assets/files/tags.yaml"):
  with open(filename, "r") as yaml_file:
    control_logic_checks, bursty_control_logic_checks, deviation_checks, devices = itemgetter(
      "control_logic_checks", 
    ...
    devices.sort(key=lambda device: device["label"])
    return control_logic_checks, bursty_control_logic_checks, deviation_checks, devices
