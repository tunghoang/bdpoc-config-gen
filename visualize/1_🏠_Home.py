...
TIME_STRINGS = {'10': '10s', '30': '30s', '60': '1m', '120': '2m', '300': '5m', '600': '10m', '1800': '30m'}
...
with tCols[2]:
    st.session_state.time_range = st.selectbox("Time Range", (10, 30, 60, 120, 300, 600, 1800), format_func=lambda sec: TIME_STRINGS[str(sec)])
with tCols[3]:
...
with col2:
    ...
    with st.spinner('Loading ...'):
        data = query_data(TIME_STRINGS[str(st.session_state.time_range)], st.session_state['selected_device_name'], st.session_state["tags"], interpolated=st.session_state.interpolated, missing_data=st.session_state.missing_data)

    if "_time" in data:
        chart = px.line(data, x="_time", y="_value", labels={"_time": "Time (s)", "_value": "Value", "_field": "Tag"}, color='_field', line_shape=LINE_SHAPE, markers=True)
